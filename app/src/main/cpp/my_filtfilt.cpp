...
void my_filtfilt::add_index_range(vectori& indices, int beg, int end, int inc)
{
	for (int i = beg; i <= end; i += inc)
		indices.push_back(i);
}

void my_filtfilt::add_index_const(vectori& indices, int value, size_t numel)
{
	while (numel--)
		indices.push_back(value);
}

void my_filtfilt::append_vector(vectord& vec, const vectord& tail)
{
	vec.insert(vec.end(), tail.begin(), tail.end());
}

vectord my_filtfilt::subvector_reverse(const vectord& vec, int idx_end, int idx_start)
{
...
inline int my_filtfilt::max_val(const vectori& vec)
{
...
void my_filtfilt::filter(vectord B, vectord A, const vectord& X, vectord& Y, vectord& Zi)
{
	if (A.empty())
...
void my_filtfilt::filtfilt(vectord B, vectord A, const vectord& X, vectord* Y)
{
